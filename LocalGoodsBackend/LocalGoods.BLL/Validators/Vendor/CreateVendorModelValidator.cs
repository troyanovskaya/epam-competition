...
namespace LocalGoods.BLL.Validators.Vendor
{
    public class CreateVendorModelValidator : AbstractValidator<CreateVendorModel>
    {
        public CreateVendorModelValidator()
        {
            ...
            RuleFor(cvm => cvm.ViberNumber).NotEmpty().Length(11, 15).Matches("^[0-9]*$");
            RuleFor(cvm => cvm.TelegramName).NotEmpty().Length(5, 100);
            RuleFor(cvm => cvm.InstagramName).NotEmpty().Length(5, 255);
